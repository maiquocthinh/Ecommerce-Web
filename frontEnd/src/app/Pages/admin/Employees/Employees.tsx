...
import { employeeGetType, employeeType } from "@/common/employee";
...
const Employees = () => {
    ...
    const adminGetAllEmployeesData = useSelector(
        (state: any) =>
            state.adminGetAllEmployeesData.data as {
                data: { list: employeeGetType[]; paging: pagingType };
            }
    );
    ...
    const [allEmployees, setAllEmployees] = useState<employeeGetType[]>([]);
    const [employeeIdUpdate, setEmployeeIdUpdate] = useState<number | null>();
    ...
    const [idEmployeeDelete, setIdEmployeeDelete] = useState<number>(0);
    const [confirmationDelete, setConfirmationDelete] =
        useState<boolean>(false);
    useEffect(() => {
        dispatch(adminAllEmployees({ pageSize: 6, pageIndex: 1 }));
    }, [dispatch]);
    useEffect(() => {
        if (
            adminGetAllEmployeesData &&
            adminGetAllEmployeesData?.data?.paging
        ) {
            setPagination({
                currentPage:
                    adminGetAllEmployeesData.data.paging.pageIndex || 1,
                totalPage: adminGetAllEmployeesData.data.paging.totalPages || 1,
                pageSize: adminGetAllEmployeesData.data.paging.pageSize || 1,
            });
        }
        if (adminGetAllEmployeesData?.data?.list) {
            setAllEmployees(adminGetAllEmployeesData?.data?.list);
        }
    }, [adminGetAllEmployeesData]);
    const handlePageChange = (newPage: number, oldPage: number) => {
        if (newPage > 0 && oldPage > 0) {
            if (searchValue.trim() !== "") {
                ...
                dispatch(
                    adminAllEmployees({ pageSize: 6, pageIndex: newPage })
                );
            }
        }
    };
    ...
    const handleResetSearchEmployee = () => {
        dispatch(
            adminAllEmployees({
                pageSize: 6,
                pageIndex: pagination.currentPage,
    ...
    const handleConfirmDeleteEmployee = (id: number) => {
        ...
        setIdEmployeeDelete(id);
    };
    const handleDeleteEmployee = async () => {
        const response = await dispatch(adminDeleteEmployee(idEmployeeDelete));
        ...
        if (response.payload?.success) {
            ...
            toast.error(
                "xóa nhân viên thất bại! vui lòng kiểm tra lại dánh sách nhân viên"
            );
            setConfirmationDelete(false);
            setIdEmployeeDelete(0);
        }
    ...
    useEffect(() => {
        if (confirmationDelete && idEmployeeDelete > 0) {
            handleDeleteEmployee();
        }
    }, [confirmationDelete, idEmployeeDelete]);
    return (
        <div className="flex flex-col p-4">
                        ...
                        <div className="flex items-center gap-2 flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                            ...
                            <div className="w-full mx-1">
                                <button
                                    ...
                                    onClick={handleResetSearchEmployee}
                                >
            ...
            {allEmployees?.length ? (
                <div style={{ overflowY: "auto" }}>
                    ...
                    {pagination && adminGetAllEmployeesData?.data?.paging && (
                        <Paginations
                            handlePageChange={handlePageChange}
                            pagination={pagination}
                            paging={adminGetAllEmployeesData.data.paging}
                        />
                    )}
                </div>
            ) : (
            ...
            {isNewEmployee && (
                <AddAndUpdateEmployee
                    ...
                    employeeIdUpdate={
                        employeeIdUpdate ? employeeIdUpdate : null
                    }
                    setEmployeeIdUpdate={setEmployeeIdUpdate}
                />
            )}
            <CenterModal
                show={isModalDelete}
